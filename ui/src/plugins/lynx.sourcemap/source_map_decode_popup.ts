      ...
      convertTraceToJsonOnly(file, async (data: string) => {
        ...
        for (let i = 0; i < traceEvents.length; i++) {
          ...
          if (event.args['args'] !== undefined) {
            event.args = event.args['args'] as Record<string, unknown>;
          }
          if (event.cat === 'jsprofile') {
            event.cat = 'jsprofile_decoded';
            const url =
              typeof event.args['url'] === 'string' ? event.args['url'] : '';
            const lineNumber =
              typeof event.args['lineNumber'] === 'number'
                ? event.args['lineNumber'] + 1
                : 1;
            const columnNumber =
              typeof event.args['columnNumber'] === 'number'
                ? event.args['columnNumber']
                : 0;
        ...
        Object.values(sourceMapState.state.sourceMapDataByUrl).forEach(
          (sourceMap) => {
    ...
    const handleChange = (e: Event) => {
      ...
      if (files && files.length > 0) {
        ...
        reader.onload = function () {
          ...
          const sourceMap = JSON.parse(str) as RawSourceMap;
          sourceMapState.edit((draft) => {
      ...
      children.push(
        m(
          ...
          Array.from(idSet).map((item) =>
            m('.source-popup-list__item', this.renderListItem(item)),
          ),
        ),
      );
      children.push(
