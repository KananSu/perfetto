...
export class VitalTimestampDetailsPanel implements TrackEventDetailsPanel {
  ...
  async load({eventId}: TrackEventSelection) {
    ...
    if (this.sliceDetail?.name !== undefined) {
      this.sliceDetail.name = this.sliceDetail.name.replace(
        TIMING_MARK_PREFIX,
        '',
      );
    }
    const pipelineId = getFirstStringArg(this.sliceDetail?.args, [
      `debug.${PIPELINE_ID}`,
      `args.${PIPELINE_ID}`,
    ]);
        ...
        (stamp) =>
          stamp.name ===
            timestamp.name.replace(
              FRAMEWORK_TIMING_START,
              FRAMEWORK_TIMING_END,
            ) && stamp.threadName === timestamp.threadName,
      );
    }
    return undefined;
  ...
  private async queryElementTree(timeStamp: number | undefined) {
    ...
    const jsonTree = getFirstStringArg(args, ['debug.content', 'args.content']);
    if (!jsonTree) {
      return undefined;
    }
